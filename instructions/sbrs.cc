...
namespace avr {
    uint8_t SBRSInstruction::GetBit(uint16_t opcode) const
    {
        return static_cast<uint8_t>(opcode & 0x07u);
    }

    uint8_t& SBRSInstruction::GetSourceRegister(CPU& cpu, uint16_t opcode) const
    {
        auto mask = 0x01F0u;
    ...
    uint16_t SBRSInstruction::GetNextOpCode(const CPU& cpu, const ProgramMemory& mem) const
    {
        auto pc = cpu.PC;
    ...
    uint16_t SBRSInstruction::GetOpCodeSize(uint16_t opcode) const
    {
    ...
    uint32_t SBRSInstruction::Execute(uint16_t opcode, ExecutionContext& ctx) const
    {
    ...
    bool SBRSInstruction::Matches(uint16_t opcode) const
    {
        auto op = static_cast<uint16_t>(OpCode::SBRS);
        auto mask = static_cast<uint16_t>(OpCodeMask::SBRS);
