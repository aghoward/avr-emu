...
class ORInstructionTests : public ::testing::Test
{

    protected:
        NoopClock clock;
        ORInstruction subject;
        ...
        uint16_t GetOpCode(uint8_t src, uint8_t dst) const
        {
            ...
            return static_cast<uint16_t>(OpCode::OR) | srcValue | dstValue;
        }
...
TEST_F(ORInstructionTests, Matches_GivenIncorrectOpCode_ReturnsFalse)
{
    auto opcode = static_cast<uint16_t>(0x1000);
    ASSERT_FALSE(subject.Matches(opcode));
}

TEST_F(ORInstructionTests, Matches_GivenOROpCode_ReturnsTrue)
{
    auto opcode = static_cast<uint16_t>(OpCode::OR);
    ASSERT_TRUE(subject.Matches(opcode));
}

TEST_F(ORInstructionTests, Execute_GivenSmallValues_DoesNotSetNegativeFlag)
{
...
TEST_F(ORInstructionTests, Execute_GivenZeroValues_SetsZeroFlag)
{
    auto [opcode, src, dst] = GetRegisters();
    ctx.cpu.R[src] = 0u;
    ctx.cpu.R[dst] = 0u;
...
TEST_F(ORInstructionTests, Execute_ClearsOverflowFlag)
{
    auto [opcode, src, dst] = GetRegisters();
    ctx.cpu.R[src] = static_cast<uint8_t>(rand() % 0x7f);
    ctx.cpu.R[dst] = static_cast<uint8_t>(rand() % 0x7f);
...
TEST_F(ORInstructionTests, Execute_GivenNegativeResult_SetsNegativeFlag)
{
    auto [opcode, src, dst] = GetRegisters();
