...
class STYInstructionTests : public ::testing::Test
{
    protected:
        NoopClock clock;
        STYInstruction subject;
        ...
        std::tuple<uint16_t, uint16_t, uint16_t, uint8_t> GetRegisters(bool pre_decrement, bool post_increment, bool has_displacement)
        {
            ...
            auto compiledOpcode = static_cast<uint16_t>(OpCode::STY)
                | ((srcIndex & 0x1Fu) << 4u)
...
TEST_F(STYInstructionTests, Matches_GivenIncorrectOpCode_ReturnsFalse)
{
    auto opcode = static_cast<uint16_t>(0x1000);
    ASSERT_FALSE(subject.Matches(opcode));
}

TEST_F(STYInstructionTests, Matches_GivenSTOpCode_ReturnsTrue)
{
    auto opcode = static_cast<uint16_t>(OpCode::STY);
    ASSERT_TRUE(subject.Matches(opcode));
}

TEST_F(STYInstructionTests, Matches_GivenSTOpCodeWithPreDecrement_ReturnsTrue)
{
    auto [opcode, offset, src, displacement] = GetRegisters(true, false, false);
    ASSERT_TRUE(subject.Matches(opcode));
}

TEST_F(STYInstructionTests, Matches_GivenSTOpCodeWithPostIncrement_ReturnsTrue)
{
    auto [opcode, offset, src, displacement] = GetRegisters(false, true, false);
    ASSERT_TRUE(subject.Matches(opcode));
}

TEST_F(STYInstructionTests, Matches_GivenSTOpCodeWithDisplacement_ReturnsTrue)
{
    auto [opcode, offset, src, displacement] = GetRegisters(false, false, true);
    ASSERT_TRUE(subject.Matches(opcode));
}

TEST_F(STYInstructionTests, Execute_StoresContentOfRegisterIntoMemory)
{
...
TEST_F(STYInstructionTests, Execute_GivenDisplacement_StoresContentOfRegisterIntoMemoryWithDisplacement)
{
    auto [opcode, offset, src, displacement] = GetRegisters(false, false, true);
    ctx.cpu.R[src] = 0xAB;
    ctx.cpu.Y = offset;
...
TEST_F(STYInstructionTests, Execute_GivenPreDecrement_StoresContentsAtPreviousAddress)
{
    auto [opcode, offset, src, displacement] = GetRegisters(true, false, false);
    ctx.cpu.R[src] = 0xAB;
    ctx.cpu.Y = offset;
    ...
    ASSERT_EQ(*ctx.cpu.Y, expectedOffset);
}

TEST_F(STYInstructionTests, Execute_GivenPostIncrement_StoresContentsAtOriginalAddress)
{
    auto [opcode, offset, src, displacement] = GetRegisters(false, true, false);
    ctx.cpu.R[src] = 0xAB;
    ctx.cpu.Y = offset;
